...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Object Type Literals")
    _ = Stdout.line!("======================================\n")

    # Test basic object type literals
    _ = test_single_case!("type Point = { x: number; y: number }")
    _ = test_single_case!("type User = { name: string; age: number }")

    # Test object types with optional properties
    _ = test_single_case!("type Config = { debug?: boolean; port?: number }")

    # Test mixed required and optional properties
    _ = test_single_case!("type Person = { name: string; age?: number; email?: string }")

    # Test empty object type
    _ = test_single_case!("type Empty = {}")

    # Test comma-separated properties
    _ = test_single_case!("type Comma = { a: string, b: number, c: boolean }")

    # Test object type with semicolon termination
    _ = test_single_case!("type Point = { x: number; y: number };")
