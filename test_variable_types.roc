...
main! = |_|
    _ = Stdout.line!("🚀 Test Variable Type Annotations")
    _ = Stdout.line!("===================================\n")

    # Test basic variable type annotations
    _ = test_single_case!("const x: number | string = undefined")
    _ = test_single_case!("let name: string = \"hello\"")
    _ = test_single_case!("var count: number = 42")

    # Test array types
    _ = test_single_case!("const items: string[] = []")
    _ = test_single_case!("let matrix: number[][] = [[1,2], [3,4]]")

    # Test object types
    _ = test_single_case!("const point: { x: number; y: number } = { x: 0, y: 0 }")

    # Test without initializer
    _ = test_single_case!("let value: boolean")
