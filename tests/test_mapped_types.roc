...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Mapped Types")
    _ = Stdout.line!("=================================\n")

    # Test basic mapped type
    _ = test_single_case!("type Readonly<T> = { readonly [K in keyof T]: T[K] };")

    # Test optional mapped type
    _ = test_single_case!("type Partial<T> = { [K in keyof T]?: T[K] };")

    # Test required mapped type (removes optional)
    _ = test_single_case!("type Required<T> = { [K in keyof T]-?: T[K] };")

    # Test mapped type with string index
    _ = test_single_case!("type StringMap<T> = { [K in string]: T };")

    # Test nullable mapped type
    _ = test_single_case!("type Nullable<T> = { [K in keyof T]: T[K] | null };")

    # Test Pick utility type
    _ = test_single_case!("type Pick<T, K extends keyof T> = { [P in K]: T[P] };")
