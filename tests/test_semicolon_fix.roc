...
main! = |_|
    _ = Stdout.line!("🚀 Test Semicolon Handling Fix")
    _ = Stdout.line!("===============================\n")

    # Test type aliases with semicolons
    _ = test_single_case!("type x = typeof y;")
    _ = test_single_case!("type UserID = string;")
    _ = test_single_case!("type Point = { x: number; y: number };")

    # Test type aliases without semicolons (should still work)
    _ = test_single_case!("type x = typeof y")
    _ = test_single_case!("type UserID = string")

    # Test multiple type aliases
    _ = test_single_case!("type A = string; type B = number;")
