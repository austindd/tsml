...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Generic Function Expressions")
    _ = Stdout.line!("==============================================\n")

    # Test the original issue case
    _ = test_single_case!("const x = function<A>(a: A): A { return a; };")

    # Test anonymous generic function
    _ = test_single_case!("const identity = function<T>(value: T) { return value; };")

    # Test multiple type parameters
    _ = test_single_case!("const mapper = function<T, U>(input: T): U { return transform(input); };")

    # Test type constraints
    _ = test_single_case!("const processor = function<T extends string>(text: T) { return text.length; };")
