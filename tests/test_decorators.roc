...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Decorators")
    _ = Stdout.line!("=============================\n")

    # Test simple class decorator
    _ = test_single_case!("@sealed class Person {}")

    # Test decorator with arguments
    _ = test_single_case!("@component({ selector: 'app' }) class AppComponent {}")

    # Test multiple decorators
    _ = test_single_case!("@sealed @log class MyClass {}")

    # Test class with decorated methods (once method decorators are supported)
    # _ = test_single_case!("class MyClass { @readonly prop = 5; }")

    # Test complex decorator
    _ = test_single_case!("@injectable() class Service {}")
