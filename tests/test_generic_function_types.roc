...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Generic Function Types")
    _ = Stdout.line!("=========================================\n")

    # Test generic function type in variable declarations
    _ = test_single_case!("const n: <A>(a: A) => B = fun")
    _ = test_single_case!("let handler: <T, U>(t: T) => U = process")

    # Test multiple type parameters with constraints
    _ = test_single_case!("const mapper: <T extends string, U extends number>(t: T) => U = transform")

    # Test with defaults
    _ = test_single_case!("let callback: <T = string>(value: T) => void = handler")
