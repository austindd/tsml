...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Intersection Types")
    _ = Stdout.line!("======================================\n")
    ...
    _ = test_single_case!("type ABC = A & B & C;")

    # Test intersection with union (precedence test)
    _ = test_single_case!("type Complex = A & B | C & D;")

    # Test parenthesized intersection
    _ = test_single_case!("type Grouped = (A & B) | C;")

    # Test intersection with arrays
    _ = test_single_case!("type ArrayIntersection = string[] & { length: 5 };")

    # Test intersection with generics
    _ = test_single_case!("type GenericIntersection = T & U;")

    # Test complex nested intersection
    _ = test_single_case!("type Nested = { a: string } & { b: number } & { c: boolean };")
