...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Enum Types")
    _ = Stdout.line!("==============================\n")

    # Test basic enum
    _ = test_single_case!("enum Color { Red, Green, Blue }")

    # Test enum with values
    _ = test_single_case!("enum Status { Active = 1, Inactive = 0 }")

    # Test enum with string values
    _ = test_single_case!("enum Direction { Up = \"UP\", Down = \"DOWN\" }")

    # Test const enum
    _ = test_single_case!("const enum Size { Small, Medium, Large }")

    # Test enum with computed values
    _ = test_single_case!("enum Values { A = 1, B = A * 2, C = B + 1 }")

    # Test enum with string literal keys
    _ = test_single_case!("enum HTTP { \"GET\" = \"get\", \"POST\" = \"post\" }")

    # Test mixed value types
    _ = test_single_case!("enum Mixed { Str = \"string\", Num = 42 }")
