...
format_type_error = |error|
    location_str =
        when error.location is
            Some loc -> "at line ${Num.to_str(loc.line)}, column ${Num.to_str(loc.column)}"
    ...
    error_msg =
        when error.error is
            InfiniteType var type ->
                ...
                Cannot create the infinite type: T${Num.to_str(var)} = ${format_type(type)}
            ...
            ArityMismatch expected actual ->
                ...
                Expected ${Num.to_str(expected)} arguments
                Actual ${Num.to_str(actual)} arguments
...
format_type_with_depth = |type, depth|
    if depth > 5 then
        "..."
    else
        when type is
            ...
            Record fields ->
                if List.is_empty fields then
                    "{}"
                else if List.len fields > 3 and depth > 2 then
                    "{ ... }"
                else
            ...
            Union types ->
                if List.len types > 3 and depth > 2 then
                    "(... union ...)"
                else
                    type_strs = List.map types |t| format_type_with_depth t (depth + 1)
                    when type_strs is
                        [] -> "never"
                        [single] -> single
            ...
            Recursive { var, body } ->
                "μT${Num.to_str(var)}.${format_type_with_depth body (depth + 1)}"
...
explain_type_mismatch = |expected, actual|
    ...
    when (expected, actual) is
        ...
        (Record expected_fields, Record actual_fields) ->
            ...
            parts =
                []
                |> |parts_| if List.is_empty(missing) then parts_ else List.append(parts_, "Missing fields: ${Str.join_with missing ", "}")
                |> |parts_| if List.is_empty (extra) then parts_ else List.append(parts_, "Extra fields: ${Str.join_with extra ", "}")
                |> |parts_| if List.is_empty (incompatible) then parts_ else List.append(parts_, "Incompatible fields: ${Str.join_with incompatible ", "}")
...
suggest_fix = |expected, actual|
    when (expected, actual) is
        ...
        (Function { params }, _) if !is_function(actual) ->
            ...
            "Try: Provide a function that takes ${Num.to_str(param_count)} argument(s)."
...
find_missing_fields = |expected, actual|
    List.keep_if(
        ...
        |e_field|
            !e_field.optional
            and !List.any(actual, |a_field| a_field.key == e_field.key),
    )
    |> List.map (.key)
...
find_incompatible_fields = |expected, actual|
    List.keep_if expected |e_field|
        List.any(
            actual,
            |a_field|
                a_field.key == e_field.key and !types_compatible(e_field.value, a_field.value),
        )
    |> List.map(.key)
...
types_compatible = |t1, t2|
    when (t1, t2) is
        (Var _, _) | (_, Var _) -> Bool.true
        (Top, _) | (_, Top) -> Bool.true
        (Bottom, _) | (_, Bottom) -> Bool.true
