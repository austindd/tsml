...
main! = |_|
    _ = Stdout.line!("🚀 Test Async/Await Functionality")
    _ = Stdout.line!("=================================\n")

    # Test async function declarations
    _ = test_single_case!("async function fetchData() {}")

    # Test async function expressions
    _ = test_single_case!("const fn = async function() {}")

    # Test await expressions
    _ = test_single_case!("async function test() { await promise }")
    _ = test_single_case!("async function complex() { const result = await fetch('url') }")

    # Test combined functionality
    _ = test_single_case!("async function process() { const data = await loadData(); return data }")
