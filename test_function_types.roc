...
main! = |_|
    _ = Stdout.line!("🚀 Test TypeScript Function Types")
    _ = Stdout.line!("==================================\n")

    # Test function type annotations
    _ = test_single_case!("let f: (a: number, b: string) => void")
    _ = test_single_case!("let handler: (event: Event) => boolean")
    _ = test_single_case!("let callback: () => void")
    _ = test_single_case!("let transform: (input: string) => number")

    # Test function type with assignment
    _ = test_single_case!("let f: (a: number, b: string) => void = (a, b) => undefined")
    _ = test_single_case!("let process: (data: any) => string = x => x.toString()")
