...
generate_constraints_helper = \node, env, next_var ->
    when node is
        ...
        NumberLiteral { value } ->
            num_val = Str.toF64 value |> Result.withDefault 0.0
                ...
                List.walk properties ([], [], next_var) \(flds, cs, var), prop ->
                    when prop is
                        Property { key, value } ->
                            ...
                            field = { key: key_name, value: val_type, optional: False }
                            (
        ...
        BlockStatement { body } | FunctionBody { body } | Program { body } ->
            process_statements body env next_var
...
instantiate_scheme = \scheme, next_var ->
    if Set.isEmpty scheme.forall then
...
apply_substitutions = \type, subs ->
    when type is
        Var v ->
            Dict.get subs v |> Option.withDefault type
        Array elem ->
        ...
        Union types ->
            Type.mk_union (List.map types \t -> apply_substitutions t subs)
        Intersection types ->
            Type.mk_intersection (List.map types \t -> apply_substitutions t subs)
        Negation inner ->
