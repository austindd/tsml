...
import pf.Stdout
import OptimizedTypes as OT exposing [TType, TNumber, TString, TBoolean, TUnknown]
...
main! = \_ ->
    ...
    _ = Stdout.line! "    TNumber == TString: $(bool_str (OT.type_eq t1 t2))"
    _ = Stdout.line! "    TNumber == TNumber: $(bool_str (OT.type_eq t1 t3))"
    ...
    _ = Stdout.line! "    TNumber == TString: $(bool_str (mt1 == mt2))"
    _ = Stdout.line! "    TNumber == TNumber: $(bool_str (mt1 == mt3))"
    ...
    when OST.lookup_symbol opt_table1 "var_50" is
        Ok sym ->
            _ = Stdout.line! "  Found var_50 in optimized table: $(Type.type_to_str sym.sym_type)"
            {}
        Err _ ->
    ...
    # Insert into cache
    cache1 = TC.cache_insert cache node1 TUnknown
    cache2 = TC.cache_insert cache1 node2 TNumber
    cache3 = TC.cache_insert cache2 node3 TString

    # Lookup from cache
    when TC.cache_lookup cache3 node2 is
        Ok (cached_type, cache4) ->
            _ = Stdout.line! "  Cache hit for node2: $(Type.type_to_str cached_type)"
    ...
    scope_table1 = when OST.add_symbol scope_table "local_var" TString Bool.false is
        Ok t ->
    ...
    union1 = OT.type_union TNumber TNumber
    _ = Stdout.line! "  Union(TNumber, TNumber) = $(OT.type_str union1)"

    union2 = OT.type_union TNumber TString
    _ = Stdout.line! "  Union(TNumber, TString) = $(OT.type_str union2)"

    intersect1 = OT.type_intersect TNumber TNumber
    _ = Stdout.line! "  Intersect(TNumber, TNumber) = $(OT.type_str intersect1)"

    intersect2 = OT.type_intersect TUnknown TString
    _ = Stdout.line! "  Intersect(TUnknown, TString) = $(OT.type_str intersect2)"
...
add_many_symbols_optimized = \table, count ->
    ...
    |> List.walk table \t, i ->
        ...
        sym_type = if i % 2 == 0 then TNumber else TString
